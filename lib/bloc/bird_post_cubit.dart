  ...
  Future<void> addBirdPost(BirdModel birdModel) async {
    emit(state.copyWith(
        status: BirdPostStatus
            .loading)); //получается что мы копируем такой же стейт только с другим статусом, а birdPosts: [] остаются те же
    ...
    Map <String, dynamic> row = {
      //DatabaseHelper.columnId: birdModel.id,
    ...
    emit(state.copyWith(
        birdPosts: birdPosts,
        status: BirdPostStatus
            .loaded)); //birdPosts уже новые, с добавленным birdModel
  }
